...
static char *new_concat(const char *a, const char *b) {
  ...
  strcat_s(new_string, strlen(a) + 1, b);
#else
  strcpy(new_string, a);
  strcat(new_string, b);
#endif
...
static FC_Rect FC_RenderLeft(FC_Font *font, FC_Target *dest, float x, float y,
                             FC_Scale scale, const char *text);
static FC_Rect FC_RenderCenter(FC_Font *font, FC_Target *dest, float x, float y,
                               FC_Scale scale, const char *text);
static FC_Rect FC_RenderRight(FC_Font *font, FC_Target *dest, float x, float y,
                              FC_Scale scale, const char *text);
...
#ifdef _WIN32
  strcpy_s(result, strlen(string) + 1, string);
#else
...
FC_Rect FC_DefaultRenderCallback(FC_Image *src, FC_Rect *srcrect,
                                 FC_Target *dest, float x, float y,
...
// FIXME: Why does the scaled offset look so wrong?
  ...
  {
    GPU_Rect r = *srcrect;
    GPU_BlitScale(src, &r, dest, x + xscale * r.w / 2.0f, y + r.h / 2.0f,
                  xscale, yscale);
  }
#else
  {
...
static FC_Rect (*fc_render_callback)(FC_Image *src, FC_Rect *srcrect,
                                     FC_Target *dest, float x, float y,
                                     float xscale,
                                     float yscale) = &FC_DefaultRenderCallback;

void FC_SetRenderCallback(FC_Rect (*callback)(FC_Image *src, FC_Rect *srcrect,
                                              FC_Target *dest, float x, float y,
                                              float xscale, float yscale)) {
  if (callback == NULL)
...
static void FC_Init(FC_Font *font) {
  ...
  font->glyph_cache =
      (FC_Image **)malloc(font->glyph_cache_size * sizeof(FC_Image *));
...
static Uint8 FC_GrowGlyphCache(FC_Font *font) {
...
#ifdef FC_USE_SDL_GPU
  GPU_Image *new_level =
      GPU_CreateImage(font->height * 12, font->height * 12, GPU_FORMAT_RGBA);
  ...
  if (new_level == NULL ||
    ...
    FC_Log("Error: SDL_FontCache ran out of packing space and could not add "
           "another cache level.\n");
#ifdef FC_USE_SDL_GPU
...
Uint8 FC_SetGlyphCacheLevel(FC_Font *font, int cache_level,
  ...
  if (cache_level == font->glyph_cache_count) {
    ...
    if (font->glyph_cache_count > font->glyph_cache_size) {
      ...
      FC_Image **new_cache;
      new_cache =
          (FC_Image **)malloc(font->glyph_cache_count * sizeof(FC_Image *));
...

  {
    ...
    for (; *source_string != '\0'; source_string = U8_next(source_string)) {
      ...
      packed = (FC_PackGlyphData(font, FC_GetCodepointFromUTF8(&buff_ptr, 0),
                                 ...
                                 surfaces[num_surfaces - 1]->h) != NULL);
      if (!packed) {
        int i = num_surfaces - 1;
        if (num_surfaces >= FC_LOAD_MAX_SURFACES) {
          // Can't do any more!
          FC_Log("SDL_FontCache error: Could not create enough cache surfaces "
...
#ifdef FC_USE_SDL_GPU
Uint8 FC_LoadFont(FC_Font *font, const char *filename_ttf, Uint32 pointSize,
                  SDL_Color color, int style)
#else
Uint8 FC_LoadFont(FC_Font *font, FC_Target *renderer, const char *filename_ttf,
...
#ifdef FC_USE_SDL_GPU
Uint8 FC_LoadFont_RW(FC_Font *font, SDL_RWops *file_rwops_ttf, Uint8 own_rwops,
                     Uint32 pointSize, SDL_Color color, int style)
#else
Uint8 FC_LoadFont_RW(FC_Font *font, FC_Target *renderer,
...
Uint8 FC_AddGlyphToCache(FC_Font *font, SDL_Surface *glyph_surface) {
...
#ifdef FC_USE_SDL_GPU
  {
    ...
    SDL_Rect destrect = font->last_glyph.rect;
    GPU_Blit(img, NULL, target, destrect.x + destrect.w / 2,
...
Uint8 FC_GetGlyphData(FC_Font *font, FC_GlyphData *result, Uint32 codepoint) {
  ...
  if (e == NULL) {
    ...
    if (cache_image == NULL) {
      FC_Log("SDL_FontCache: Failed to load cache image, so cannot add new "
             "glyphs!\n");
      return 0;
    }
...
static void FC_RenderAlign(FC_Font *font, FC_Target *dest, float x, float y,
  ...
  for (iter = ls; iter != NULL; iter = iter->next) {
    ...
    if (width > 0 && FC_GetWidth(font, "%s", line) > width) {
      ...
      for (word_iter = words->next, spaces_iter = spaces->next;
           word_iter != NULL && spaces_iter != NULL;
...
static void FC_DrawColumnFromBuffer(FC_Font *font, FC_Target *dest, FC_Rect box,
                                    int *total_height, FC_Scale scale,
                                    FC_AlignEnum align) {
  int y = box.y;
  FC_StringList *ls, *iter;
...
FC_Rect FC_DrawBox(FC_Font *font, FC_Target *dest, FC_Rect box,
                   const char *formatted_text, ...) {
  Uint8 useClip;
  if (formatted_text == NULL || font == NULL)
  ...
  FC_DrawColumnFromBuffer(font, dest, box, NULL, FC_MakeScale(1, 1),
                          FC_ALIGN_LEFT);
...
FC_Rect FC_DrawBoxAlign(FC_Font *font, FC_Target *dest, FC_Rect box,
...
FC_Rect FC_DrawBoxScale(FC_Font *font, FC_Target *dest, FC_Rect box,
...
FC_Rect FC_DrawBoxColor(FC_Font *font, FC_Target *dest, FC_Rect box,
...
FC_Rect FC_DrawBoxEffect(FC_Font *font, FC_Target *dest, FC_Rect box,
  ...
  FC_DrawColumnFromBuffer(font, dest, box, NULL, effect.scale,
                          effect.alignment);
...
FC_Rect FC_DrawColumn(FC_Font *font, FC_Target *dest, float x, float y,
  ...
  FC_DrawColumnFromBuffer(font, dest, box, &total_height, FC_MakeScale(1, 1),
                          FC_ALIGN_LEFT);

  return FC_MakeRect(box.x, box.y, width, total_height);
}

FC_Rect FC_DrawColumnAlign(FC_Font *font, FC_Target *dest, float x, float y,
...
FC_Rect FC_DrawColumnScale(FC_Font *font, FC_Target *dest, float x, float y,
...
FC_Rect FC_DrawColumnColor(FC_Font *font, FC_Target *dest, float x, float y,
...
FC_Rect FC_DrawColumnEffect(FC_Font *font, FC_Target *dest, float x, float y,
...
static FC_Rect FC_RenderCenter(FC_Font *font, FC_Target *dest, float x, float y,
      ...
      *c = '\0';
      result = FC_RectUnion(
          FC_RenderLeft(font, dest,
                        x - scale.x * FC_GetWidth(font, "%s", str) / 2.0f, y,
                        scale, str),
          result);
      *c = '\n';
      c++;
  ...
  result = FC_RectUnion(
      FC_RenderLeft(font, dest,
                    x - scale.x * FC_GetWidth(font, "%s", str) / 2.0f, y, scale,
                    str),
      result);
...
FC_Rect FC_DrawScale(FC_Font *font, FC_Target *dest, float x, float y,
                     FC_Scale scale, const char *formatted_text, ...) {
  if (formatted_text == NULL || font == NULL)
    return FC_MakeRect(x, y, 0, 0);
...
FC_Rect FC_DrawAlign(FC_Font *font, FC_Target *dest, float x, float y,
                     FC_AlignEnum align, const char *formatted_text, ...) {
  if (formatted_text == NULL || font == NULL)
    return FC_MakeRect(x, y, 0, 0);
...
FC_Rect FC_DrawColor(FC_Font *font, FC_Target *dest, float x, float y,
                     SDL_Color color, const char *formatted_text, ...) {
  if (formatted_text == NULL || font == NULL)
    return FC_MakeRect(x, y, 0, 0);
...
FC_Rect FC_DrawEffect(FC_Font *font, FC_Target *dest, float x, float y,
                      FC_Effect effect, const char *formatted_text, ...) {
  if (formatted_text == NULL || font == NULL)
    return FC_MakeRect(x, y, 0, 0);
...
FC_Rect FC_GetCharacterOffset(FC_Font *font, Uint16 position_index,
  ...
  if (formatted_text == NULL || column_width == 0 || position_index == 0 ||
      font == NULL)
    return result;
  ...
  for (iter = ls; iter != NULL;) {
    ...
    ++num_lines;
    for (line = iter->value; line != NULL && *line != '\0';
...
FC_Rect FC_GetBounds(FC_Font *font, float x, float y, FC_AlignEnum align,
                     FC_Scale scale, const char *formatted_text, ...) {
  FC_Rect result = {x, y, 0, 0};
...
Uint16 FC_GetPositionFromOffset(FC_Font *font, float x, float y,
                                int column_width, FC_AlignEnum align,
                                const char *formatted_text, ...) {
  FC_StringList *ls, *iter;
  Uint8 done = 0;
  ...
  for (iter = ls; iter != NULL; iter = iter->next) {
    ...
    for (line = iter->value; line != NULL && *line != '\0';
      ...
      if (FC_GetGlyphData(font, &glyph_data,
        ...
        if (FC_InRect(x, y,
                      FC_MakeRect(current_x, current_y, glyph_data.rect.w,
                                  glyph_data.rect.h))) {
          done = 1;
          break;
        }
