...
void TextBox_render(TextBox *self, SDL_Renderer *renderer) {
  ...
  FC_Draw(font, renderer, self_x, self_y, TextBox_gettext(self));
...
void TextBox_render_bg_color(TextBox *self, SDL_Renderer *renderer,
                             SDL_Color color) {
  if (self->is_hidden)
    return;
  int screen_w, screen_h;
...
void TextBox_get_screen_coords(TextBox *self, SDL_Renderer *renderer, int *x,
                               int *y) {
  int screen_w, screen_h;
  SDL_GetRendererOutputSize(renderer, &screen_w, &screen_h);
...
int TextBox_is_mouse_inside(TextBox *self, SDL_Renderer *renderer, int x,
                            int y) {
  FC_Font *font = Fonts_getfont(self->font_size);

  int height = FC_GetLineHeight(font);
  int width = (self->box_width != 0 ? self->box_width
                                    : FC_GetWidth(font, TextBox_gettext(self)));
  int self_x, self_y;
  TextBox_get_screen_coords(self, renderer, &self_x, &self_y);

  return (x >= self_x) && (x <= self_x + width) && (y >= self_y) &&
         (y <= self_y + height);
}
